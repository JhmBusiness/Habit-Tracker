...
function NewPostModal({ habitId, category }: NewPostProps) {
  ...
  async function returnHabitStreakData() {
    try {
      ...
      setValue("streakMilestone", currentStreak);
    } catch (error) {
  ...
  const onSubmit = (data: FormInputs) => {
    ...
    if (content.length > 240) {
      toast.error("Paragraph is too long.");
      return;
    }
    mutateForm.mutate({
      title,
...
function PostCategorySelectionModal() {
  const { closeModal } = useModal();
  const { habitsData } = useUserHabits();

  const modalBtnLabels: { [key: string]: string } = {
    fitness: "Fitness",
    sleepSchedule: "Sleep Hygiene",
    learning: "Learning",
    reading: "Reading",
    hydration: "Hydration",
    bedHygiene: "Bed Hygiene",
  ...
  return (
    <>
      ...
      <p className="text-grey mb-6">
        Select the habit you&apos;d like to make a post about.
      </p>
      {/* Habit category selection */}
      <div className="gap-3 flex flex-wrap mb-6 sm:gap-4">
        {habitsData.map((habit: habit) => (
          <NewPostHabitCategoryBtn
            key={habit.category}
...
function EditPostModal({ postId }: EditPostProps) {
  const { closeModal } = useModal();

  const { postsData, loading } = useUserPosts();
  const postToRender = postsData!.filter((el) => el.id === postId);
  ...
  const {
    ...
    defaultValues: {
      ...
      content: content,
      category,
      streakMilestone: milestone_streak,
  ...
  // -----------------------
  //
  // Get all details for the post and use them as default values for the form. Then create a mutate fn to update the post with the new values.
  //
  // -----------------------

  const mutateForm = useUpdateUserPost();
